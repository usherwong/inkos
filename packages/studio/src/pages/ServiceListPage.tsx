...
import { GROUP_ORDER } from "../constants/service-groups";
...
const GROUP_LABEL_KEYS: Record<EndpointGroup, StringKey> = {
  overseas: "svc.group.overseas",
  china: "svc.group.china",
  aggregator: "svc.group.aggregator",
  local: "svc.group.local",
  codingPlan: "svc.group.codingPlan",
};
const GROUP_SHORT_KEYS: Record<EndpointGroup, StringKey> = {
  overseas: "svc.groupShort.overseas",
  china: "svc.groupShort.china",
  aggregator: "svc.groupShort.aggregator",
  local: "svc.groupShort.local",
  codingPlan: "svc.groupShort.codingPlan",
};
    ...
    >
      <button onClick={onClick} className="flex flex-1 flex-col gap-2 text-left">
        ...
        <span className="text-xs text-muted-foreground/60">
          {svc.connected ? t("svc.connected") : t("svc.notConfigured")}
        </span>
      </button>
      {quickLinks.length > 0 && (
...
function CoverConfigCard() {
  ...
  useEffect(() => {
    ...
    void fetchJson<CoverConfigPayload>("/cover/config")
      ...
      .catch((error) => {
        if (cancelled) return;
        setStatus("error");
        setMessage(error instanceof Error ? error.message : t("svc.coverLoadFailed"));
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus("saved");
      setMessage(t("svc.coverSaved"));
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : t("svc.coverSaveFailed"));
    }
  };
  ...
  return (
    <section className="rounded-xl border border-border/50 bg-card/50 p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-medium text-foreground">{t("svc.coverTitle")}</h2>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1.5">
          <span className="block text-xs font-medium text-muted-foreground/70">{t("svc.service")}</span>
          <select
            value={service}
        ...
        <label className="space-y-1.5">
          <span className="block text-xs font-medium text-muted-foreground/70">{t("svc.coverModel")}</span>
          <select
            value={model}
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className="text-foreground">{t("svc.title")}</span>
      </div>

      <h1 className="font-serif text-2xl">{t("svc.title")}</h1>
        ...
        {GROUP_ORDER.map((group) => {
          ...
          return (
            ...
            >
              ...
              {t(GROUP_SHORT_KEYS[group])} {groupCounts[group]}
            </button>
          );
        })}
        {selectedGroups.size > 0 && (
      ...
      <label className="inline-flex cursor-pointer select-none items-center gap-2 text-xs text-muted-foreground">
        ...
        />
        <span>{t("svc.onlyConnected")} ({connectedCount})</span>
      </label>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {canCreateCustom && (
              ...
              >
                <Plus size={18} />
                <span className="text-xs">{t("svc.addCustom")}</span>
              </button>
            )}
          </div>
        </section>

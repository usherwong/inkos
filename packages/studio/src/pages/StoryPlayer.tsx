...
export function StoryPlayer({
  ...
  if (!node) return <div className="text-destructive">{isZh ? "节点缺失：" : "Missing node: "}{currentId}</div>;
      ...
      {isEnding ? (
        <div className="border rounded-xl p-8 text-center space-y-4" data-testid="player-ending">
          ...
          <div className={c.muted} data-testid="player-unlocked">
            {t("play.endingsUnlocked")} {unlocked.length} / {graph.endings.length}
          </div>
        ...
        <div className="space-y-2" data-testid="player-choices">
          ...
          {choices.length === 0 && <div className="text-destructive" data-testid="player-deadend">{t("play.deadEnd")}</div>}
        </div>
      )}
